...
describe('solr.js', () => {
  ...
  describe('querySOLR', () => {
    it('should return response object with docs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { id: 'job1', url: 'https://test.com/1', cif: '47473595' },
        { id: 'job2', url: 'https://test.com/2', cif: '47473595' }
      ]));

      const result = await solr.querySOLR('47473595');
    ...
    it('should throw when SOLR_AUTH is missing', async () => {
      delete process.env.SOLR_AUTH;
      await expect(solr.querySOLR('47473595')).rejects.toThrow('SOLR_AUTH not set in environment');
      process.env.SOLR_AUTH = 'test:test';
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('47473595')).rejects.toThrow('SOLR query error: 500');
    });
  });
  ...
  describe('queryCompanySOLR', () => {
    it('should return company data', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { id: '47473595', company: 'GUSTURI DIVINE S.R.L.', brand: 'gusturi-divine' }
      ]));

      const result = await solr.queryCompanySOLR('id:47473595');

      expect(result.numFound).toBe(1);
      expect(result.docs[0].brand).toBe('gusturi-divine');
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.queryCompanySOLR('id:47473595')).rejects.toThrow('SOLR company query error: 401');
    });
  });
  ...
  describe('deleteJobsByCIF', () => {
    it('should delete all jobs for a CIF', async () => {
      ...
      await expect(solr.deleteJobsByCIF('47473595')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobsByCIF('47473595')).rejects.toThrow('SOLR delete error: 500');
    });
  });
  ...
  describe('Data Integrity', () => {
    it('should not have duplicate URLs for same CIF', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://test.com/job1', title: 'Job 1', cif: '47473595' },
        { url: 'https://test.com/job2', title: 'Job 2', cif: '47473595' }
      ]));
    ...
    it('should have valid CIF format for all jobs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://test.com/1', title: 'Job 1', cif: '47473595' },
        { url: 'https://test.com/2', title: 'Job 2', cif: '47473595' }
      ]));
    ...
    it('should have valid status values', async () => {
      ...
      mockFetch.mockResolvedValue(makeSolrResponse(3, [
        { url: 'https://test.com/1', title: 'Job 1', cif: '47473595', status: 'scraped' },
        { url: 'https://test.com/2', title: 'Job 2', cif: '47473595', status: 'verified' },
        { url: 'https://test.com/3', title: 'Job 3', cif: '47473595', status: 'published' }
      ]));
